...
from archs.modules.vit import load_encoder
from archs.modules.roberta import load_decoder
...
class Nexus(nn.Module):
    def __init__(self, config):
        ...
        nexus_config = json.load(open('configs/nexus.json', 'r'))[config['nexus_model']]
        phobert_config = RobertaConfig.from_dict(nexus_config['phobert_model'])

        # self.tokenizer = RobertaTokenizer.from_pretrained(nexus_config['roberta_model']['model_name'])
        self.tokenizer = AutoTokenizer.from_pretrained("vinai/phobert-base-v2")
        self.expert_encoder = load_encoder(nexus_config['vit_model'], experts=self.experts, image_resolution=config['image_resolution'])
        self.text_decoder = load_decoder("vinai/phobert-base-v2", config=phobert_config)
